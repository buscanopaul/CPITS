...
if(Input::exists())
{
	if(isset($_POST['process']))
	{
		if(Token::check(Input::get('token')))
		{
			...
			{
				$verify = DB::getInstance()->query("SELECT id FROM checkin WHERE id = ? AND status = ?", array(Input::get('id'), 2));
				...
				else
				{
					...
					try
					{
						DB::getInstance()->update('checkin', Input::get('id'), array(
							'status' => $status,
	...
	if(isset($_POST['assigned']))
	{
		if(Token::check(Input::get('token')))
		{
			...
			if($validation->passed())
			{
				...
				$verify = DB::getInstance()->query("SELECT id FROM checkin WHERE id = ? AND status = ?", array(Input::get('id'), 1));
